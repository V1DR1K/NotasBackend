...
Inversion Cripto: `POST /api/finance/crypto/transfers` pasa un importe en ARS desde MercadoPago a la cuenta Cripto; `GET /api/finance/crypto/summary` devuelve el saldo disponible, el costo registrado y las posiciones; `GET /api/finance/crypto/investments` lista compras; `POST /api/finance/crypto/investments` registra `{ "date":"2026-09-03", "assetCode":"BTCUSDT", "amountUsd":1800 }`; y `DELETE /api/finance/crypto/investments/{id}` revierte una asignacion. El MVP admite BTCUSDT, SOLUSDT, ETHUSDT y PEPEUSDT, guarda la conversion ARS/USD del momento y deja el rendimiento para una futura fuente de cotizaciones cripto.
