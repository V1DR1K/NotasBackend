...
@RestController @RequestMapping("/api/day-entries") public class DayController {
    ...
    @GetMapping public PageResponse<DayDtos.Response> list(@RequestParam(required = false) LocalDate date, @RequestParam(required = false) LocalDate from, @RequestParam(required = false) LocalDate to, @RequestParam(required = false) String statusCode, @RequestParam(required = false, name = "feeling") List<String> feelings, @PageableDefault(size = 20, sort = "date", direction = org.springframework.data.domain.Sort.Direction.DESC) Pageable page) { return service.list(CurrentUser.id(), date, from, to, statusCode, feelings, page); }
