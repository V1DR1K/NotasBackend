...
@RestController @RequestMapping("/api") public class FileController {
    ...
    @GetMapping("/files") public PageResponse<FileDtos.FileResponse> list(@RequestParam(required = false) UUID folderId, @RequestParam(required = false) String kind, @RequestParam(required = false) String name, @RequestParam(required = false) String search, @RequestParam(required = false) LocalDate from, @RequestParam(required = false) LocalDate to, @PageableDefault(size = 20, sort = "createdAt", direction = org.springframework.data.domain.Sort.Direction.DESC) Pageable page) { return service.list(CurrentUser.id(), new FileDtos.Filters(folderId, kind == null ? null : FileKind.fromValue(kind), search != null ? search : name, from, to), page); }
    @GetMapping("/files/{id}") public FileDtos.FileResponse get(@PathVariable UUID id) { return service.get(CurrentUser.id(), id); }
    @PostMapping(value = "/files", consumes = MediaType.MULTIPART_FORM_DATA_VALUE) @ResponseStatus(HttpStatus.CREATED) public FileDtos.FileResponse upload(@RequestPart("file") MultipartFile file, @RequestParam(value = "folderId", required = false) UUID folderId, @RequestParam(value = "name", required = false) String name, @RequestParam(value = "description", required = false) String description) { return service.upload(CurrentUser.id(), folderId, file, name != null ? name : description); }
