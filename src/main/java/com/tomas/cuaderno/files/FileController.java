...
@RestController @RequestMapping("/api") public class FileController {
    ...
    @GetMapping("/files/{id}/download") public ResponseEntity<Resource> download(@PathVariable UUID id) { FileService.Download result = service.download(CurrentUser.id(), id); return ResponseEntity.ok().contentType(MediaType.parseMediaType(result.metadata().getMimeType())).header(HttpHeaders.CONTENT_DISPOSITION, ContentDisposition.attachment().filename(result.metadata().getName()).build().toString()).contentLength(result.metadata().getSizeBytes()).body(result.resource()); }
