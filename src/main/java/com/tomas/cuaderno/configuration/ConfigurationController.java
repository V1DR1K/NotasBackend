...
public class ConfigurationController {
    ...
    @GetMapping("/event-categories") public List<ConfigurationDtos.ConfigOptionResponse> eventCategories() { return service.list(CurrentUser.id(), ConfigKind.EVENT_CATEGORY); }
    @PostMapping("/event-categories") @ResponseStatus(HttpStatus.CREATED) @PreAuthorize("hasRole('ADMIN')") public ConfigurationDtos.ConfigOptionResponse createEventCategory(@Valid @RequestBody ConfigurationDtos.OptionRequest request) { return service.createOption(CurrentUser.id(), ConfigKind.EVENT_CATEGORY, request); }
    @PatchMapping("/event-categories/{code}") @PreAuthorize("hasRole('ADMIN')") public ConfigurationDtos.ConfigOptionResponse patchEventCategory(@PathVariable String code, @Valid @RequestBody ConfigurationDtos.PatchRequest request) { return service.patch(CurrentUser.id(), ConfigKind.EVENT_CATEGORY, code, request); }
    @DeleteMapping("/event-categories/{code}") @ResponseStatus(HttpStatus.NO_CONTENT) @PreAuthorize("hasRole('ADMIN')") public void deleteEventCategory(@PathVariable String code) { service.delete(CurrentUser.id(), ConfigKind.EVENT_CATEGORY, code); }
}
